...
clearMatchResultsButton.addEventListener("click", () => clearMatchResults("Job analysis results have been cleared.", { confirm: true }));
...
function handleSubmit(event) {
  ...
  if (hasCurrentMatchResults()) {
    setMatchStatus("A new experience was added. Analyze again to refresh the recommendations.", { showReanalyze: true });
  }
}
...
function handleGenerateStar() {
  const description = form.experienceDescription.value.trim();
  if (!description) {
    setStarGenerationStatus("Please add an experience summary before generating STAR.", true);
    return;
  }
  ...
  setStarGenerationStatus(generatedStar.resultWarning ? "This experience is missing a clear outcome. Add metrics, volume, time saved, or stakeholder feedback if possible." : "STAR has been generated from the experience summary. You can still edit it.", Boolean(generatedStar.resultWarning));
}
...
function deleteExperience(id) {
  ...
  if (hasCurrentMatchResults() && (!matchedExperienceIds.length || matchedExperienceIds.includes(id))) {
    clearMatchResults("The experience was deleted. Related recommendations were cleared. Please analyze again.");
  }
}
...
function useSampleJobDescription() {
  ...
  if (hasCurrentMatchResults()) {
    clearMatchResults("The JD was updated. Previous analysis results were cleared. Please analyze again.");
  }
...
function handleJobDescriptionInput() {
  ...
  if (!description) {
    clearMatchResults("The JD is empty. Related analysis results were removed.");
    return;
  }
  if (currentJobDescription && description !== currentJobDescription) {
    clearMatchResults("The JD was changed. Previous analysis results were cleared. Please analyze again.");
  }
}

function clearJobDescriptionAndResults() {
  jobDescription.value = "";
  clearMatchResults("The JD was cleared. Related analysis results were removed.");
  jobDescription.focus();
}
...
function analyzeJobMatch() {
  ...
  if (!description) {
    ...
    matchResults.innerHTML = `<p class="match-empty">${language === "en" ? "Please paste responsibilities or requirements before analyzing." : "请先粘贴岗位职责或任职要求，再进行分析。"}</p>`;
    setMatchStatus("");
    return;
  }
  ...
  if (!experiences.length) {
    ...
    currentJobDescription = "";
    matchedExperienceIds = [];
    matchResults.innerHTML = `<p class="match-empty">${language === "en" ? "Please add at least one experience before matching." : "请先添加至少一条经历，再进行岗位匹配。"}</p>`;
    setMatchStatus("");
    return;
  }
...
function clearMatchResults(message = "", options = {}) {
  if (options.confirm && hasCurrentMatchResults()) {
    const confirmed = window.confirm("Clear the current job analysis results?");
    if (!confirmed) return;
  }
...
function exportData() {
  ...
  setBackupStatus(`Exported ${latestExperiences.length} experience${latestExperiences.length === 1 ? "" : "s"}.`);
  window.alert("Data exported. The file only contains experiences saved in this browser.");
}
...
function setSelectedImportFile(file) {
  if (!file) {
    ...
    selectedImportFileName.textContent = "No file selected";
    return;
  }

  selectedImportFile = file;
  selectedImportFileName.textContent = `已选择：${file.name}`;
...
function importData() {
  if (!selectedImportFile) {
    setBackupStatus("Please choose or drop a .json file first.", true);
    return;
  }

  if (!isJsonFile(selectedImportFile)) {
    setBackupStatus("Please choose a .json file.", true);
    return;
  }
  ...
  reader.addEventListener("load", async () => {
    try {
      ...
      if (!importedBackup.rawExperiences.length) {
        setBackupStatus("This file does not contain experience data.", true);
        return;
      }

      if (!importedExperiences.length) {
        throw new Error("Import failed. Please upload a JSON file exported by this tool or the demo data file.");
      }
      ...
      saveExperiences();
      renderExperiences();
      clearMatchResults("Data imported. Please analyze the JD again.");
      setBackupStatus(hasImportAnalytics
        ? `Imported ${importedExperiences.length} experience${importedExperiences.length === 1 ? "" : "s"}.`
        : `Imported ${importedExperiences.length} experience${importedExperiences.length === 1 ? "" : "s"}.`
      );
    } catch (error) {
      console.log("[import] failed:", error);
      setBackupStatus(error.message || "Import failed. Please upload a JSON file exported by this tool or the demo data file.", true);
    } finally {
      selectedImportFile = null;
  ...
  reader.addEventListener("error", () => {
    setBackupStatus("The file content is not valid JSON.", true);
    selectedImportFile = null;
    importDataInput.value = "";
    selectedImportFileName.textContent = "No file selected";
  });
...
function parseImportedBackup(fileContent) {
  ...
  if (!Array.isArray(rawExperiences)) {
    throw new Error("No experiences array was found in this JSON file.");
  }
  ...
  return new Promise((resolve) => {
    ...
    backdrop.innerHTML = `
      <div class="import-modal" role="dialog" aria-modal="true" aria-labelledby="importModeTitle">
        <h3 id="importModeTitle">Local data already exists. Choose an import mode:</h3>
        <p>This will import ${escapeHtml(importCount)} experience${importCount === 1 ? "" : "s"}. Merge keeps current local experiences; replace overwrites the current list with the JSON file.</p>
        <div class="import-modal-actions">
          <button class="primary-button" type="button" data-import-mode="merge">Merge Import</button>
          <button class="danger-button" type="button" data-import-mode="replace">Replace Local Data</button>
          <button class="secondary-button" type="button" data-import-mode="cancel">Cancel</button>
        </div>
      </div>
    `;
...
function clearLocalData() {
  const confirmed = window.confirm("Clear local data? This deletes all local experiences and current match results.");
  if (!confirmed) return;
  ...
  selectedImportFileName.textContent = "No file selected";
  setStarGenerationStatus("");
  clearMatchResults("Local data has been cleared. Job analysis results were removed.");
  setBackupStatus("Local data has been cleared. Job analysis results were removed.");
  updateInterfaceLanguage();
  renderExperiences();
}
